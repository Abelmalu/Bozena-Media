...
func (authRepo *AuthRepository) Register(ctx context.Context, user *model.User) (*model.User, error) {
	...
	query := `INSERT INTO users(name,username,email,password) VALUES($1,$2,$3,$4) RETURNING id,role,name`
	if err := authRepo.DB.QueryRowContext(ctx, query, user.Name, user.Username, user.Email, user.Password).Scan(&newUser.ID, &newUser.Role,&newUser.Username); err != nil {
